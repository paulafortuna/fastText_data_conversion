...
flags.DEFINE_bool(
    "do_classify_new_data", False,
    "Whether to run the model in new data.")

flags.DEFINE_string("new_data_task_name", None, "The name of the task of the new data.")

flags.DEFINE_string("new_data_path", None, "The path of the new data.")
...
def main(_):
  ...
  processors = {
      ...
      "one": classifier_utils.Offen_1_Processor,
  ...
  if not FLAGS.do_train and not FLAGS.do_eval and not FLAGS.do_predict and not FLAGS.do_classify_new_data:
    raise ValueError(
  ...
  if FLAGS.do_predict:
    """
      ...
      df_result = pd.DataFrame(columns=['id', 'text', 'label', 'predict_class0', 'predict_class1'])

      for example, prediction in zip(predict_examples, predictions):
          print(type(example))
          if(not isinstance(example, classifier_utils.PaddingInputExample)):
              df_result = df_result.append({'text': example.text_a,
                                            'id': example.guid,
                                            'label': example.label,
                                            'predict_class0': prediction['probabilities'][0],
                                            'predict_class1': prediction['probabilities'][1]}, ignore_index=True)
      RES_DIR = 'gs://bert_example_2020_02/results_F1_albert'
      df_result.to_csv('{}/{}_{}'.format(RES_DIR, task_name,'predictions.csv'), index = False)

      print(df_result)

      df_F1 = pd.DataFrame()
      df_F1 = df_F1.append(pd.Series(np.asarray(precision_recall_fscore_support(pd.to_numeric(df_result['label']),round(df_result['predict_class1']),pos_label=1, average='binary'))),ignore_index=True)
      df_F1 = df_F1.append(pd.Series(np.asarray(precision_recall_fscore_support(pd.to_numeric(df_result['label']),round(df_result['predict_class1']), average='macro'))),ignore_index=True)
      df_F1 = df_F1.append(pd.Series(np.asarray(precision_recall_fscore_support(pd.to_numeric(df_result['label']),round(df_result['predict_class1']), average='micro'))),ignore_index=True)
      df_F1 = df_F1.append(pd.Series(np.asarray(precision_recall_fscore_support(pd.to_numeric(df_result['label']),round(df_result['predict_class1']), average='weighted'))),ignore_index=True)

      df_F1.columns = ['precision', 'recall', 'f1', 'f1_type']
      df_F1['f1_type'] = ['binary','macro','micro','weighted']

      df_F1.to_csv('{}/{}_{}'.format(RES_DIR, task_name,'F1_scores.csv'), index = False)
      print(df_F1)
    """
    print("hello")


  if FLAGS.do_classify_new_data:

      """
      # for all vs all model evaluation
      print("************************************")
      print("************************************")
      print("************************************")
      print("************************************")
      print("Enter classify new data")
      print(FLAGS.new_data_task_name)
      print("Enter classify model")
      print(task_name)


      processor_classify_new_data = processors[FLAGS.new_data_task_name](
          use_spm=True if FLAGS.spm_model_file else False,
          do_lower_case=FLAGS.do_lower_case)

      import pandas as pd

      predict_examples = processor_classify_new_data.get_test_examples(FLAGS.data_dir)
      num_actual_predict_examples = len(predict_examples)
      if FLAGS.use_tpu:
          # TPU requires a fixed batch size for all batches, therefore the number
          # of examples must be a multiple of the batch size, or else examples
      ...
      for example, prediction in zip(predict_examples, predictions):
          ...
          if(not isinstance(example, classifier_utils.PaddingInputExample)):
              df_result = df_result.append({'text': example.text_a,
                                            ...
                                            'predict_class0': prediction['probabilities'][0],
                                            'predict_class1': prediction['probabilities'][1]}, ignore_index=True)
      RES_DIR = 'gs://bert_example_2020_02/results_F1_albert_all_models_'
      df_result.to_csv('{}_{}/{}_{}'.format(RES_DIR, FLAGS.new_data_task_name, task_name,'predictions.csv'), index = False)
      ...
      df_F1.to_csv('{}_{}/{}_{}'.format(RES_DIR, FLAGS.new_data_task_name, task_name, 'F1_scores.csv'), index = False)
      print(df_F1)
      ...
      if FLAGS.do_classify_new_data:
        ...
        def get_test_examples_local(file_path_num):

          lines = []
          dataset = pd.read_csv('./training-albert/NaziData/dataset_without_nan_sentences_' + str(file_path_num) +'.tsv', sep='\t', encoding='utf-8', error_bad_lines=False)
          ...
          for (i, line) in enumerate(lines):
            ...
            label = '0'
            examples.append(
              classifier_utils.InputExample(guid=guid, text_a=text_a, text_b=None, label=label))
          return examples


        #predict_examples = processor_classify_new_data.get_test_examples(FLAGS.data_dir)
        predict_examples = get_test_examples_local(FLAGS.new_data_path)
        num_actual_predict_examples = len(predict_examples)
        if FLAGS.use_tpu:
            # TPU requires a fixed batch size for all batches, therefore the number
            # of examples must be a multiple of the batch size, or else examples
        ...
        for example, prediction in zip(predict_examples, predictions):
            ...
            if (not isinstance(example, classifier_utils.PaddingInputExample)):
                df_result = df_result.append({'text': example.text_a,
                                              ...
                                              'predict_class0': prediction['probabilities'][0],
                                              'predict_class1': prediction['probabilities'][1]}, ignore_index=True)
        RES_DIR = 'gs://bert_example_2020_02/results_sentences_albert_all_models_'
        df_result.to_csv('{}_new_data_{}/{}_{}'.format(RES_DIR, task_name, FLAGS.new_data_task_name, 'new_data_predictions' + str(FLAGS.new_data_path) + '.csv'), index=False)
